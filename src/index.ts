...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        const player = button.dataset.type as string
        ...
        setTimeout(
            () => {
                ...
                setTimeout(() => {
                    ...
                    if (result === 'player') playerBtn.classList.add('winner')
                    if (result === 'computer') computerBtn.classList.add('winner')
...
function compareWinner(player: string, computer: string): 'player' | 'computer' | 'tie' {
    if (player === 'paper') {
...
function randFace(arr: typeof rps) {
    if (arr && Array.isArray(arr) && arr.length) return arr[Math.floor(Math.random() * arr.length)]
